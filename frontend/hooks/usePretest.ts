...
export default function usePretest() {
    ...
    const fetchPretestGrade = async (coursesId: string) => {
        ...
        setError(null);
        try {
            const res = await fetch(`${process.env.API_URL}/student/courses/${coursesId}/answer/pretest`, {
                method: "GET",
                credentials: "include",
            });
    ...
    const createPretestAnswer = async (answers: { question: string; answer: string }[], courseId: string) => {
        ...
        setError(null);
        try {
            const res = await fetch(`${process.env.API_URL}/student/courses/${courseId}/answer/pretest`, {
                method: "POST",
                headers: {
