...
export class PretestAnswerController {
  ...
  @Post()
  submitAnswer( @Request() req, @Param('courseId') courseId: string, @Body() answers: CreatePretestAnswerDto[]) {
    return this.pretestAnswerService.submitPretestAnswer(req.user.id, courseId, answers );
  }
  
  @Get()
  gradeAnswer(@Request() req, @Param('courseId') courseId: string) {
