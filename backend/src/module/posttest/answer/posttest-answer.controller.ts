...
export class PosttestAnswerController {
  ...
  submitAnswers(
    ...
    @Body() answers: CreatePosttestAnswerDto[],
  ) {
    return this.posttestAnswerService.submitPosttestAnswer(
      req.user.id,
      courseId,
      answers,
  ...
  @Get()
  gradeAnswers(@Request() req: User, @Param('courseId') courseId: string) {
    return this.posttestAnswerService.gradePosttestAnswers(
      req.user.id,
      courseId,
    );
  }
